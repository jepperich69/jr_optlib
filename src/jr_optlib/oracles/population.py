...
def certify_population_margins(df, constraints, weight_col="x", tol: float = 1e-6):
    ...
    for cs in constraints:
        ...
        results.append(
            OracleResult(
                ...
                tol=tol,
                certifies=False,
                detail=f"max_abs_gap={gap:.3e} total_abs_gap={total_gap:.3e}",
            )
        )
    ...
    results.append(
        OracleResult(
            ...
            tol=tol,
            certifies=ok_all,
            detail=f"max margin residual across {len(constraints)} constraints",
        )
    )
